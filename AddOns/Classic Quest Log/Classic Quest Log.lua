...
function cql:OnEvent(event)
   ...
   if event=="PLAYER_LOGIN" then
	  ...
	  ClassicQuestLogSettings.UndockWindow = true -- forcing to be undocked for now
...
function cql:HideWindow()
   ...
   if ClassicQuestLogSettings.UndockWindow then
      ...
      NewHideUIPanel(ClassicQuestLog)
   end
end
...
function cql:OptionsUpdate()
   ...
   cql.optionsFrame.UndockWindow:Disable()
   cql.optionsFrame.UndockWindow.text:SetTextColor(0.5,0.5,0.5)
...
function cql:UpdateDocking()
	if true then return end -- undocking is always enabled for now
